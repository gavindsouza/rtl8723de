...
static int retriveFromFile(const char *path, u8 *buf, u32 sz)
{
	...
	if (path && buf) {
		...
		if (0 == ret) {
			...
			oldfs = get_fs();
			/*
			* pre 5.1.x : set_fs(get_ds())
			* get_ds() returned address limits for process
			*
			* post 5.1.x: set_fs(KERNEL_DS)
			* KERNEL_DS => kernel data segment
			* other option would be USER_DS but the former fits the purpose?
			*/
			set_fs(KERNEL_DS);
